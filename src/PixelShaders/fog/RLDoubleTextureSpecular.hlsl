struct PS_INPUT
{
    ...
    float2 spec_uv : TEXCOORD2;
    float2 spec : TEXCOORD3;
};
...
void main(in PS_INPUT input, out PS_OUTPUT output)
{
    ...
    float3 tex_spec = tex2D(TextureSampler, input.spec_uv);
    ...
    float3 specular = saturate(mul(4, input.spec.x * tex_spec));
    float3 spec_diffuse = saturate(specular + diffuse);
    float3 result = saturate(spec_diffuse + input.v1);
